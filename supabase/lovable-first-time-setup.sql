...
-- Target project_id: gicsknanyctuxjfxvmqo
...
-- chavruta_requests: public request form (anon may submit a pending request);
-- only an admin can read the raw table (it holds PII). Approved rows are exposed
-- publicly, with contact details masked, only through the RPC below.
...
CREATE POLICY "anyone can submit chavruta request" ON public.chavruta_requests
...
CREATE TRIGGER chavruta_requests_updated BEFORE UPDATE ON public.chavruta_requests
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
